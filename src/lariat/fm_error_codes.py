...
# List of error codes taken from:
# https://support.claris.com/s/article/Error-codes-for-Custom-Web-Publishing-1503692934814
...
FM_ERROR_CODES = {
    ...
    5: (
        "Command is invalid (for example, a Set Field script step does not have a"
    ...
    205: (
        "User does not have access privileges to create new records; import will"
        " overwrite existing data"
    ),
    206: "User does not have password change privileges, or file is not modifiable",
    207: (
        "User does not have sufficient privileges to change database schema, or file is"
        " not modifiable"
    ),
    ...
    505: (
        "Value in field is not an existing value in the database file as required in"
        " validation entry options"
    ),
    506: (
        "Value in field is not listed on the value list specified in validation entry"
        " option"
    ),
    ...
    710: (
        "Unable to update QuickTime file reference because the database file is"
        " read-only"
    ),
    ...
    723: (
        "You do not have permission to add records to or modify records in the target"
        " table"
    ),
    ...
    726: (
        "There are more records in the import file than in the target table. Not all"
        " records were imported"
    ),
    727: (
        "There are more records in the target table than in the import file. Not all"
        " records were updated"
    ),
    729: "Errors occurred during import. Records could not be imported",
    730: (
        "Unsupported Excel version. (Convert file to Excel 7.0 (Excel 95), Excel 97,"
        " 2000, or XP format and try again)"
    ),
    ...
    735: (
        "This file type cannot be displayed as a picture. It will be inserted and"
        " displayed as a file"
    ),
    ...
    1214: (
        "AVERAGE, COUNT, EXTEND, GETREPETITION, MAX, MIN, NPV, STDEV, SUM and"
        " GETSUMMARY: expression found where a field alone is needed"
    ),
    ...
    1400: (
        "ODBC client driver initialization failed; make sure the ODBC client drivers"
